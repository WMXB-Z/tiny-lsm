...
std::vector<uint8_t> Block::encode(bool with_hash) {
    ...
    // 4. 写入检验码
    if (with_hash) {
        // 计算哈希值并写入：创建一个临时的 hash 函数对象，然后立刻调用
        // 即从 encoded 中取出前 N-4 字节 → 转成字符串 → 计算 hash 值
        //  std::string 是“拥有一段字符内存的对象（RAII 管理者）”
        //  std::string_view 是“只读、不拥有内存的引用（指针 + 长度）”
        ...
        memcpy(encoded.data() + num_pos + sizeof(uint16_t), &hash_value, sizeof(uint32_t));
    }
    return encoded;
}
...
std::shared_ptr<Block> Block::decode(const std::vector<uint8_t>& encoded, bool with_hash) {
    ...
    // 1. 安全性检查1（block起码有：[num_elements][CRC]）
    ...
    // 3. 安全性检查2（验证block的起码有：[offsets...][num_elements]）
...
// 根据offsets的索引下标，获得k-v在data段中的位置
...
int Block::adjust_idx_by_tranc_id(size_t idx, uint64_t tranc_id) {
    ...
    if (tranc_id != 0) {
        ...
        if (cur_tranc_id <= tranc_id) { // 当前记录可见，向前查找更接近的目标（tranc_id越大越靠前）
            size_t prev_idx = idx;
            ...
            size_t next_idx = idx + 1;   // 当前记录不可见，向后查找
        ...
        // （因为二分查找中有相同元素时，只会找到最接近mid的，但他不一定是最新的）
        size_t prev_idx = idx;
        while (prev_idx > 0 && is_same_key(prev_idx - 1, target_key)) {
            prev_idx--;
        }
...
// 判断offsets中idx所指向的key和target_key是否相同
...
Block::iters_preffix(uint64_t tranc_id, const std::string& preffix) {
    ...
    // 直接调用已实现的谓词区间查询函数get_monotony_predicate_iters
...
Block::get_monotony_predicate_iters(uint64_t tranc_id, std::function<int(const std::string&)> predicate) {
    ...
    return std::make_optional<std::pair<std::shared_ptr<BlockIterator>,
                                        std::shared_ptr<BlockIterator>>>(it_begin, it_end);
}
