...
LSMEngine::LSMEngine(std::string path) : data_dir(path) {
    ...
    // 初始化 VLog (总是open, 这是为了保证WiscKey 阈值为0，也能保证文件存在，从而正常重启读取)
    ...
    if (std::filesystem::exists(path)) {
        ...
        for (const auto& entry : std::filesystem::directory_iterator(path)) {
            ...
            // 提取SST ID
            std::string id_str =
                filename.substr(4, dot_pos - 4);  // 4 for "sst_"
            if (id_str.empty()) {
                continue;
            }
            ...
            next_sst_id =
                (std::max)(sst_id, next_sst_id);  // 记录目前最大的 sst_id
            cur_max_level =
                (std::max)(level, cur_max_level);  // 记录目前最大的 level
            std::string sst_path = get_sst_path(sst_id, level);
            auto sst = SST::open(sst_id, FileObj::open(sst_path, false),
                                 block_cache, vlog_);
            spdlog::info(
                "LSMEngine--"
        ...
        for (auto& [level, sst_id_list] : level_sst_ids) {
            // 对各层 sst_id_list 排序以便方便后续查询，排序要求是sst_id大的在前面
...
std::optional<std::pair<std::string, uint64_t>> LSMEngine::get(
    ...
    // ? 3. 遍历 L1 及以上各层, 对每层做二分查找确定 key所在的 SST 文件 
    // ? 注意: value 为空字符串表示 key 已被删除, 此时返回nullopt
    ...
    for (auto& sst_id : level_sst_ids[0]) {
        //  level-0中的 sst_id 是按从大到小的顺序排列, sst_id 越大, 表示是越晚刷入的, 优先查询
...
LSMEngine::get_batch(const std::vector<std::string>& keys, uint64_t tranc_id) {
    ...
    for (auto& [key, value] : results) {
        if (value.has_value())  { continue; } // 已找到，跳过
        for (auto& sst_id : level_sst_ids[0]) {
            auto& sst = ssts[sst_id];
...
std::optional<std::pair<std::string, uint64_t>> LSMEngine::sst_get_(
    const std::string& key, uint64_t tranc_id) {
    // TODO: Lab 4.2 sst 内部查询，不加锁 (专门在SST中进行查询的接口，不查memtable)（已通过）
...
uint64_t LSMEngine::put(const std::string& key, const std::string& value, uint64_t tranc_id) {
    ...
    // ? 若 memtable 总大小 >= LsmTolMemSizeLimit 则调用 flush() 并返回其结果(结果是新的最大事务Id)
    // ? 否则返回 0
    ...
    // std::cout << "MemSizeLimit大小：" << TomlConfig::getInstance().getLsmTolMemSizeLimit() << "\n";
...
LSMEngine::lsm_iters_monotony_predicate(uint64_t tranc_id, std::function<int(const std::string&)> predicate) {
    // TODO: Lab 4.7 谓词查询
    // ? 1. 从 memtable 查询: memtable.iters_monotony_predicate(tranc_id, predicate) 
    // ? 2. 遍历所有 SST, 对每个 SST 调用sst_iters_monotony_predicate 
    ...
    std::shared_ptr<HeapIterator> ss_iter_ptr =
...
Level_Iterator LSMEngine::begin(uint64_t tranc_id) {
    // TODO: Lab 4.7 返回指向的begin位置的层级指针Level_Iterator
    return Level_Iterator(shared_from_this(), tranc_id);
}

Level_Iterator LSMEngine::end() {
    // TODO: Lab 4.7 返回指向的end位置的层级指针Level_Iterator
    // ? 返回空的 Level_Iterator{}
...
void LSMEngine::full_compact(size_t src_level) {
    ...
    // ? 2. 根据 src_level 是否为 0分别调用 full_l0_l1_compact 或 full_common_compact 
    // ? 3. 删除旧 SST文件并从 ssts/level_sst_ids 中移除记录
    // ? 4. 将新的 SST 加入level_sst_ids[src_level+1] 并排序 
    // ? 5. 更新 cur_max_level
...
std::vector<std::shared_ptr<SST>> LSMEngine::full_l0_l1_compact(
    ...
    // TODO: Lab 4.5 负责完成 level-0 和 level-1 的 full compact
    // ? L0 各 SST 的 key 有重叠, 需要先通过 SstIterator::merge_sst_iterator合并 
    // ? 再用 TwoMergeIterator 与 L1 的 ConcactIterator 合并 
    // ? 最后调用gen_sst_from_iter 生成新的 SST 文件 (目标大小 = PerMemSizeLimit *SstLevelRatio)
...
std::vector<std::shared_ptr<SST>> LSMEngine::full_common_compact(
    ...
    // TODO:如果目标 level 的下一级 level+1 不存在, 则为底层的level,可以清理掉删除标记
...
std::vector<std::shared_ptr<SST>> LSMEngine::gen_sst_from_iter(
    ...
    // ? 当 estimated_size >= target_sst_size 时 (注意不能在相同 key的不同版本之间切分) 
    ...
    while (iter.is_valid() && !iter.is_end()) {
        ...
        // keep_all_versions = true时，sst各层就可能出现重复key，但本程序不使用这个模式！
    ...
    if (new_sst_builder.real_size() > 0) {
        ...
        std::string sst_path = get_sst_path(sst_id, target_level);
        auto new_sst =
            new_sst_builder.build(sst_id, sst_path, this->block_cache);
        new_ssts.push_back(new_sst);
...
LSM::LSM(std::string path)
    ...
    // ? 4. 调用 tran_manager_->init_new_wal() 开启新的 WAL文件准备接收新写入

    // 1、绑定关键指针
    tran_manager_->set_engine(engine);
    engine->set_tran_manager(tran_manager_);
    // 2、根据wal恢复未flush至memtable中
    recover_from_wal();
    // 3、初始化并绑定wal对象（开启wal文件）
