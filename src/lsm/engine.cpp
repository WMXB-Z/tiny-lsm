...
LSMEngine::LSMEngine(std::string path) : data_dir(path) {
  ...
  // ? 3. 若目录不存在则创建
  // ? 4. 初始化 VLog: vlog_ = VLog::open(data_dir +
  // "/vlog.data")，这是用于键值分离 ? 5. 遍历目录加载所有已存在的 SST 文件: ?
  // - 文件名格式: sst_{id}.{level} ?    - 调用 SST::open 并记录到 ssts 和
  // level_sst_ids ?    - 维护 next_sst_id 和 cur_max_level ? 6. next_sst_id
  // 自增 ? 7. 对各层 sst_id_list 排序; L0 层需要 reverse (越大的 id 越新,
  // 优先查询) 初始化日志
  init_spdlog_file();
  ...
  if (std::filesystem::exists(path)) {
    ...
    for (const auto &entry : std::filesystem::directory_iterator(path)) {
      ...
      auto sst =
          SST::open(sst_id, FileObj::open(sst_path, false), block_cache, vlog_);
      spdlog::info("LSMEngine--"
...
uint64_t LSMEngine::put(const std::string &key, const std::string &value,
  ...
  spdlog::trace("LSMEngine--put({}, {}, {}) inserted into memtable", key, value,
                tranc_id);
...
uint64_t LSMEngine::remove(const std::string &key, uint64_t tranc_id) {
  ...
  spdlog::trace("LSMEngine--remove({}, {}) marked as deleted in memtable", key,
                tranc_id);
...
LSM::LSM(std::string path)
  ...
  // engine->remove() ? 4. 调用 tran_manager_->init_new_wal() 开启新的
  // WAL文件准备接收新写入
