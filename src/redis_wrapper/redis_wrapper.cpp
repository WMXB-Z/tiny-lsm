...
// Helper functions
RedisWrapper::RedisWrapper(const std::string &db_path) { this->lsm = std::make_unique<LSM>(db_path); }
...
std::vector<std::string> get_fileds_from_hash_value(const std::optional<std::string> &field_list_opt) {
    ...
    if (!field_list.empty()) {
        ...
        field_list = field_list.substr(preffix.size(), field_list.size() - preffix.size());
    }
    std::vector<std::string> fields;
    std::istringstream iss(field_list);
    std::string token;
    while (std::getline(iss, token, TomlConfig::getInstance().getRedisFieldSeparator())) {
...
std::string get_hash_value_from_fields(const std::vector<std::string> &fields) {
    ...
    for (size_t i = 0; i < fields.size(); ++i) {
        if (i > 0) oss << TomlConfig::getInstance().getRedisFieldSeparator();
        oss << fields[i];
    }
    return oss.str();
}
...
std::string get_zset_key_socre(const std::string &key, const std::string &score) {
    std::ostringstream oss;
    oss << std::setw(TomlConfig::getInstance().getRedisSortedSetScoreLen()) << std::setfill('0') << score;

    std::string formatted_score = oss.str();

    std::string res = TomlConfig::getInstance().getRedisSortedSetPrefix() + key + "_SCORE_" + formatted_score;
    return res;
}

inline std::string get_zset_key_elem(const std::string &key, const std::string &elem) {
    return TomlConfig::getInstance().getRedisSortedSetPrefix() + key + "_ELEM_" + elem;
}
...
inline std::string get_set_member_key(const std::string &key, const std::string &elem) {
...
bool RedisWrapper::expire_zset_clean(const std::string &key, std::shared_lock<std::shared_mutex> &rlock) {
    ...
    if (is_expired(expire_query, nullptr)) {
        ...
        auto result_elem = this->lsm->lsm_iters_monotony_predicate(
            0, [&preffix](const std::string &elem) { return -elem.compare(0, preffix.size(), preffix); });
...
// ************************* Redis Command *************************
// 基础操作
std::string RedisWrapper::set(std::vector<std::string> &args) { return redis_set(args[1], args[2]); }
std::string RedisWrapper::get(std::vector<std::string> &args) { return redis_get(args[1]); }
std::string RedisWrapper::del(std::vector<std::string> &args) { return redis_del(args); }
std::string RedisWrapper::incr(std::vector<std::string> &args) { return redis_incr(args[1]); }

std::string RedisWrapper::decr(std::vector<std::string> &args) { return redis_decr(args[1]); }

std::string RedisWrapper::expire(std::vector<std::string> &args) { return redis_expire(args[1], args[2]); }

std::string RedisWrapper::ttl(std::vector<std::string> &args) { return redis_ttl(args[1]); }
...
std::string RedisWrapper::hget(std::vector<std::string> &args) { return redis_hget(args[1], args[2]); }

std::string RedisWrapper::hdel(std::vector<std::string> &args) { return redis_hdel(args[1], args[2]); }

std::string RedisWrapper::hkeys(std::vector<std::string> &args) { return redis_hkeys(args[1]); }

// 链表操作
std::string RedisWrapper::lpush(std::vector<std::string> &args) { return redis_lpush(args[1], args[2]); }
std::string RedisWrapper::rpush(std::vector<std::string> &args) { return redis_rpush(args[1], args[2]); }
std::string RedisWrapper::lpop(std::vector<std::string> &args) { return redis_lpop(args[1]); }
std::string RedisWrapper::rpop(std::vector<std::string> &args) { return redis_rpop(args[1]); }
std::string RedisWrapper::llen(std::vector<std::string> &args) { return redis_llen(args[1]); }
...
// 有序集合操作
std::string RedisWrapper::zadd(std::vector<std::string> &args) { return redis_zadd(args); }

std::string RedisWrapper::zrem(std::vector<std::string> &args) { return redis_zrem(args); }

std::string RedisWrapper::zrange(std::vector<std::string> &args) { return redis_zrange(args); }

std::string RedisWrapper::zcard(std::vector<std::string> &args) { return redis_zcard(args[1]); }

std::string RedisWrapper::zscore(std::vector<std::string> &args) { return redis_zscore(args[1], args[2]); }
std::string RedisWrapper::zincrby(std::vector<std::string> &args) { return redis_zincrby(args[1], args[2], args[3]); }

std::string RedisWrapper::zrank(std::vector<std::string> &args) { return redis_zrank(args[1], args[2]); }

// 无序集合操作
std::string RedisWrapper::sadd(std::vector<std::string> &args) { return redis_sadd(args); }

std::string RedisWrapper::srem(std::vector<std::string> &args) { return redis_srem(args); }

std::string RedisWrapper::sismember(std::vector<std::string> &args) { return redis_sismember(args[1], args[2]); }

std::string RedisWrapper::scard(std::vector<std::string> &args) { return redis_scard(args[1]); }

std::string RedisWrapper::smembers(std::vector<std::string> &args) { return redis_smembers(args[1]); }
...
std::string RedisWrapper::redis_expire(const std::string &key, std::string seconds_count) {
    std::unique_lock<std::shared_mutex> lock(redis_mtx);  // 写锁
    std::string expire_key = get_explire_key(key);
    ...
    if (key_query.has_value()) {
        ...
        if (expire_query.has_value()) {
            ...
            if (is_expired(expire_query, &now_time_t)) {
                ...
                // 没有过期
                auto now = std::chrono::system_clock::now();
                return ":" + std::to_string(std::stoll(expire_query.value()) - now_time_t) + "\r\n";
            }
        } else {
            // 没有设置过期时间, 返回 -1
            return ":-1\r\n";
...
// 哈希操作
std::string RedisWrapper::redis_hset_batch(const std::string &key,
                                           std::vector<std::pair<std::string, std::string>> &field_value_pairs) {
/*  例如命执行令：HSET user name 张三 age 20 sex male
    LSM中将存入：
    user       		       → "$name$age$male"
...
std::string RedisWrapper::redis_hset(const std::string &key, const std::string &field, const std::string &value) {
...
std::string RedisWrapper::redis_hget(const std::string &key, const std::string &field) {
    std::shared_lock<std::shared_mutex> rlock(redis_mtx);  // 读锁
    bool is_expired = expire_hash_clean(key, rlock);
...
std::string RedisWrapper::redis_hdel(const std::string &key, const std::string &field) {
    std::shared_lock<std::shared_mutex> rlock(redis_mtx);  // 读锁线判断是否过期
    bool is_expired = expire_hash_clean(key, rlock);
    ...
    rlock.unlock();
    std::unique_lock<std::shared_mutex> lock(redis_mtx);  // 写锁, 如果过期的话, 读锁在 expire_hash_clean 中已经被释放了
...
// 链表操作
std::string RedisWrapper::redis_lpush(const std::string &key, const std::string &value) {
    /*
        例如命执行令：会在“列表左侧插入新元素”
        LPUSH mylist A
        LPUSH mylist B
    ...
    */
    ...
    if (!list_value.empty()) {
        list_value = value + TomlConfig::getInstance().getRedisListSeparator() + list_value;
    } else {
        list_value = value;
    }

    lsm->put(key, list_value);
    return ":" + std::to_string(split(list_value, TomlConfig::getInstance().getRedisListSeparator()).size()) + "\r\n";
}

std::string RedisWrapper::redis_rpush(const std::string &key, const std::string &value) {
    std::shared_lock<std::shared_mutex> rlock(redis_mtx);  // 读锁
    bool is_expire = expire_list_clean(key, rlock);
    ...
    std::string list_value = list_opt.value_or("");
    if (!list_value.empty()) {
        list_value = list_value + TomlConfig::getInstance().getRedisListSeparator() + value;
    } else {
        list_value = value;
    }
...
std::string RedisWrapper::redis_lpop(const std::string &key) {
    ...
    std::vector<std::string> elements = split(list_opt.value(), TomlConfig::getInstance().getRedisListSeparator());
    if (elements.empty()) {
        return "$-1\r\n";  // 表示链表为空
    }
    ...
    if (elements.empty()) {
        ...
        lsm->put(key, join(elements, TomlConfig::getInstance().getRedisListSeparator()));
    }
...
std::string RedisWrapper::redis_lrange(const std::string &key, int start, int stop) {
    std::shared_lock<std::shared_mutex> rlock(redis_mtx);  // 读锁
    bool is_expire = expire_list_clean(key, rlock);
    ...
    if (stop < 0) stop += elements.size();
    if (start < 0) start = 0;
    if (stop >= elements.size()) stop = elements.size() - 1;
...
std::string RedisWrapper::redis_zrange(std::vector<std::string> &args) {
    ...
    auto result_elem = this->lsm->lsm_iters_monotony_predicate(
        0, [&preffix_score](const std::string &elem) { return -elem.compare(0, preffix_score.size(), preffix_score); });
...
std::string RedisWrapper::redis_zscore(const std::string &key, const std::string &elem) {
    std::shared_lock<std::shared_mutex> rlock(redis_mtx);  // 读锁
    bool is_expired = expire_zset_clean(key, rlock);
...
std::string RedisWrapper::redis_zincrby(const std::string &key, const std::string &increment, const std::string &elem) {
    std::shared_lock<std::shared_mutex> rlock(redis_mtx);  // 读锁
    bool is_expired = expire_zset_clean(key, rlock);
...
std::string RedisWrapper::redis_zrank(const std::string &key, const std::string &elem) {
    std::shared_lock<std::shared_mutex> rlock(redis_mtx);  // 读锁
    bool is_expired = expire_zset_clean(key, rlock);
...
std::string RedisWrapper::redis_sismember(const std::string &key, const std::string &member) {
    std::shared_lock<std::shared_mutex> rlock(redis_mtx);  // 读锁
    bool is_expired = expire_set_clean(key, rlock);
...
std::string RedisWrapper::redis_smembers(const std::string &key) {
    ...
    auto result_elem = this->lsm->lsm_iters_monotony_predicate(
        0, [&prefix](const std::string &elem) { return -elem.compare(0, prefix.size(), prefix); });
