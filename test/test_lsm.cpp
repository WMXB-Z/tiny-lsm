...
TEST_F(LSMTest, TrancIdTest) {
    // 注意是 LSMEngine 而不是 LSM，因为 LSMEngine 才能手动控制事务id
    // LSMEngine lsm(test_dir);
    auto engine = std::make_shared<LSMEngine>(test_dir);
    auto tranmanager = std::make_shared<TranManager>(test_dir);
    tranmanager->set_engine(engine);
    engine->set_tran_manager(tranmanager);
    ...
    for (int i = 0; i < 20; i++) {
        ...
        oss_key << "key" << std::setw(2) << std::setfill('0') << i;
        std::string key = oss_key.str();
        engine->put(key, "tranc1", 1);
    }
    engine->flush();

    // key00-key10 再插入, 此时事务id为2
    for (int i = 0; i < 10; i++) {
        std::ostringstream oss_key;
        oss_key << "key" << std::setw(2) << std::setfill('0') << i;
        std::string key = oss_key.str();
        engine->put(key, "tranc2", 2);
    }
...
// 验证LSM 在“大规模写入 + 删除 + 重启后”是否还能正确恢复数据
TEST_F(LSMTest, BigPersistence) {
    ...
    int num = 2000000;
    {
        ...
        for (int i = 0; i <= num; ++i) {
            ...
            kvs[key] = value;
            if (i % 400000 == 0 && i != 0) {
                std::cout << "lsm put key at i = " << i << "......"
                          << std::endl;
            }
            // 删除能被10整除的key（实际上不会删，是插入空值key）
            if (i % 10 == 0 && i != 0) {
                std::string del_key = "key" + std::to_string(i - 10);
                lsm.remove(del_key);
                kvs.erase(del_key);
...
void random_change_vector(std::vector<int>& a) {
    ...
    for (int i = 0; i < swap_cnt; i++) {
        ...
        int x2 = ((i * i * pri[(pri_pos + 1) % 5] * pri[(pri_pos + 1) % 5] +
                   i * i + 31) %
                  (a.size()));
        std::swap(a[x1], a[x2]);
    }
...
// 刻意打乱顺序来验证LSM 的顺序语义 + 删除覆盖逻辑 + 重启恢复。
TEST_F(LSMTest, BigPersistence2) {
    ...
    // idx1在放入lsm前打乱key，idx2在放入lsm后打乱key
