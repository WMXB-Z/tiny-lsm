 ...
 * --------------------------------------------------------------------------------
 * | data block | ... | data block |    metadata   |       ...        |    ... |
 * --------------------------------------------------------------------------------
 *
 * 其中, metadata 是一个数组加上一些描述信息, 数组中每个元素由一个data
 * Block的元数据信息编码形成 MetaEntry MetaEntry 结构如下:
 * ---------------------------------------------------------------------------------------------------
 * | offset(32) | 1st_key_len(16) | 1st_key(1st_key_len) | last_key_len(16) |last_key(last_key_len) |
 * ---------------------------------------------------------------------------------------------------
 *
 * Meta Section 的总体结构如下:
 * ---------------------------------------------------------------
...
public:
    // 打开sst文件，读取必要的元信息记录在sst实例中 (vlog defaults to nullptr for backward compat)
...
private:
    ...
    std::string first_key;
    std::string last_key;
    std::vector<BlockMeta> meta_entries;  // 存放Meta Section中meta entry
    std::vector<uint8_t> data;  // 存放Block Section中的data block的数组
    size_t block_size;
    std::shared_ptr<BloomFilter> bloom_filter;
...
public:
    ...
    SSTBuilder(size_t block_size, bool has_bloom, std::shared_ptr<VLog> vlog,
