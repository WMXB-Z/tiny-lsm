...
// 使用的跳表基于双向链表
struct SkipListNode {
    std::string key_;    // 节点存储的键。按key升序排列
    std::string value_;  // 节点存储的值
    uint64_t tranc_id_;  // 事务 id，值越大表示优先级越高，优先级高的排在前面（key相同时，按tranc_id逆序排列）
    std::vector<std::shared_ptr<SkipListNode>>
        forward_;  // 指向不同层级的下一个节点的指针数组
    std::vector<std::weak_ptr<SkipListNode>>
        backward_;  // 指向不同层级的上一个节点的指针数组（为了防止循环引用，这里使用weak_ptr）
    ...
    bool operator==(const SkipListNode& other) const {
        return key_ == other.key_ && value_ == other.value_ && tranc_id_ == other.tranc_id_;
    }
    ...
    bool operator<(const SkipListNode& other) const {
        if (key_ == other.key_) {
            // key 相等时，trans_id 更大的优先级更高(在次序上排在前面)
...
private:
    ...
    std::shared_ptr<std::shared_lock<std::shared_mutex>> lock;  // 持有读锁, 整个迭代器有效期间都持有读锁
};
...
class SkipList {
private:
    std::shared_ptr<SkipListNode> head;           // 跳表的头节点，不存储实际数据，用于遍历跳表
    int max_level;      // 跳表的最大层级数，限制跳表的高度
    int current_level;  // 跳表当前的实际层级数，动态变化
    size_t size_bytes = 0;  // 跳表当前占用的内存大小（字节数），用于跟踪内存使用
    // std::shared_mutex rw_mutex; // ! 目前看起来这个锁是冗余的,
    // 在上层控制即可, 后续考虑是否需要细粒度的锁
...
public:
    ...
    ~SkipList() {
        ...
        // Disconnect all nodes before cleanup to avoid stack overflow in case
        // of deep recursion during shared_ptr destruction
        auto current = head;
