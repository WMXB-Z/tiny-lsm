...
#define ASYNC_REDIS_SERVER_LOG_INFO(msg) std::cout << "[INFO] " << msg << std::endl;
#define ASYNC_REDIS_SERVER_LOG_DEBUG(msg) /* std::cout << "[DEBUG] " << msg << std::endl; */  // 在发布版本中可以注释掉

using asio::ip::tcp;

// 安全地获取对端地址：连接已断开时 remote_endpoint() 会抛异常，
// 这里统一兜底，避免异常从 asio 回调逃逸导致整个服务进程崩溃
static std::string peer_info(tcp::socket &socket) {
  try {
...
class RedisSession : public std::enable_shared_from_this<RedisSession> {
   public:
    RedisSession(tcp::socket socket, RedisWrapper &redis_db) : socket_(std::move(socket)), redis_(redis_db) {}

    void start() {
        ASYNC_REDIS_SERVER_LOG_INFO("Connection from " << socket_.remote_endpoint().address().to_string() << ":"
   ...
   private:
        ...
        asio::async_read_until(
            socket_, buffer_, "\r\n", [this, self](const asio::error_code &ec, std::size_t bytes_transferred) {
                if (!ec) {
                    ...
                    for (int i = 0; i < numElements * 2; ++i) {  // 每个元素有两个部分：长度行和数据行
    ...
    void do_write(const std::string &response) {
        ...
        asio::async_write(socket_, asio::buffer(response),
                          [this, self, response](const asio::error_code &ec, std::size_t /*bytes_transferred*/) {
                              if (!ec) {
    ...
    std::string handleRequest(const std::string &request) {
        ...
        for (int i = 0; i < numElements; ++i) {
            if (pos >= request.size() || request[pos] != '$') {
                ASYNC_REDIS_SERVER_LOG_DEBUG("pos = " << pos << ", i = " << i << ", last args = "
            ...
            try {
                ...
                ASYNC_REDIS_SERVER_LOG_DEBUG("-ERR Protocol error: invalid bulk string length\r\n");
                return "-ERR Protocol error: invalid bulk string length\r\n";
            }
            pos = next_n_pos + 1;  // 跳过 '$' 值 \r\n
            if (pos + len > request.size()) {
                ASYNC_REDIS_SERVER_LOG_DEBUG("-ERR Protocol error: bulk string length exceeds request size\r\n");
    ...
    case OPS::CONFIG:
      return config_handler(args, redis_);
    case OPS::SET:
...
class RedisServer {
   public:
    RedisServer(asio::io_context &io_context, unsigned short port)
        : acceptor_(io_context, tcp::endpoint(tcp::v4(), port)), redis_("example_db") {  // 初始化 RedisWrapper
        do_accept();
    }
...
int main() {
    try {
        ...
        reset_log_level("off");        // 引擎此时已初始化日志
